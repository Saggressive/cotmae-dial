...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        import numpy as np
        attns = attention_probs.cpu().numpy()
        attns = 0.999999 * attns + (0.000001 / attns.shape[-1])
        entropies_cls = (attns * np.log(attns))[:, :,0 ].sum(-1)
        print(-1*entropies_cls.mean())

        # This is actually dropping out entire tokens to attend to, which might
        # seem a bit unusual, but is taken from the original Transformer paper.
        attention_probs = self.dropout(attention_probs)
        # att = torch.sum(attention_probs,dim=1)

        # att = attention_probs[:,0,:,:]
        # att = att[:,0,:]
        # att = att[att!=0]
        # log = torch.log(att)
        # res = torch.sum(log)*1.0/log.shape[0]
