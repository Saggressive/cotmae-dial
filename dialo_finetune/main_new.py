...
@torch.no_grad()
def eval_data_mul(model,eval_loader,args,mode="test"):
    model.eval()
    cos = nn.CosineSimilarity(dim=-1)
    all_scores,all_labels = [],[]
    ...
    res = [*total_correct,total_examples]
    print(res)
    ...
    for step,batch in enumerate(train_loader):
        ...
        if global_step%args.eval_steps==0:
            if args.dataset == "ubuntu" or args.eval_mode=="test":
                res = eval_data_mul(model, dev_loader, args,args.eval_mode)
                collection_list = [torch.zeros_like(res) for _ in range(dist.get_world_size())]
                dist.all_gather(tensor_list=collection_list, tensor=res.contiguous())
                x = collection_list[0]
                for index in range(1,len(collection_list)):
                    y = collection_list[index]
                    x = torch.add(x,y)
                x = x.cpu().tolist()
                R10_1 = round(((x[0]/x[3])*100), 2)
                R10_2 = round(((x[1]/x[3])*100), 2)
                R10_5 = round(((x[2]/x[3])*100), 2)
                print(x)
                logger.info(f"rank:{utils.get_rank()},epoch:{epoch},global_step:{global_step},R10_1:{R10_1},R10_2:{R10_2},R10_5:{R10_5}")
...
def main():
    ...
    if args.distributed: 
        ...
        sampler_dev = torch.utils.data.DistributedSampler(
            dev_set, num_replicas=num_tasks, rank=sampler_rank, shuffle=False
        )
    else:
        sampler_train = torch.utils.data.RandomSampler(train_set)
        # sampler_dev = torch.utils.data.RandomSampler(dev_set)
